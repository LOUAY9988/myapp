...
def write_crash_log(text: str):
    try:
        ...
        pass
    try:
        # try also to write to common Downloads folder on Android so user can access it
        download_path = os.path.join("/storage/emulated/0", "Download", "myapp_crash.log")
        with open(download_path, "w", encoding="utf-8") as f:
            f.write(text)
    except Exception:
        pass

# Try to import KivyMD; if it fails, create a minimal fallback app that shows an error message.
USE_KIVYMD = True
try:
    ...
    write_crash_log("KivyMD import failed:\n" + tb)

if not USE_KIVYMD:
    class FallbackApp(App):
        def build(self):
            root = BoxLayout(orientation='vertical', padding=10, spacing=10)
            root.add_widget(Label(text='KivyMD library not available in this APK.', halign='center'))
            root.add_widget(Label(text='Please rebuild with kivymd in requirements or install the correct APK.', halign='center'))
    ...
    if __name__ == '__main__':
        FallbackApp().run()
    # stop further execution if fallback was used
else:
    ...
    class MyApp(MDApp):
        def build(self):
            try:
                ...
                # If a crash log exists from previous run, show it
                try:
                    if os.path.exists(CRASH_LOG):
                        with open(CRASH_LOG, "r", encoding="utf-8") as f:
                            contents = f.read()
                        # show a dialog with contents (shortened)
                        msg = contents if len(contents) < 4000 else contents[-4000:]
                        Clock.schedule_once(lambda dt: MDDialog(title="Crash log found", text=msg, size_hint=(0.9, 0.9)).open(), 0.5)
                except Exception:
                    pass
    ...
    if __name__ == "__main__":
        try:
            ...
            tb = traceback.format_exc()
            write_crash_log(tb)
            # if we can't start the Kivy event loop, attempt to save to downloads and exit
            try:
